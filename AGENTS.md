...
- gpt-oss-20b reference (MXFP4, `ggml-org/gpt-oss-20b-GGUF`, same machine, 2026-07-21, 3 clean GPU runs): **12.1 GB loads reliably** in ≈ 112 s, so raw size is not the binding limit on a 16 GB card — but it decodes at only ≈ 2.7 t/s (≈ 1.0 t/s app-reported, ≈ 49 s avg TTFT), ~13× slower than dense Gemma-4-12B despite activating only ~3.6B params/token. MXFP4 has no optimized WebGPU path in wllama. All three runs ended `too_slow` *before* the 10-question set finished, and not via the speed threshold (`--threshold 0` was set): a long writing question returned nothing inside the 300 s per-question timeout → `no_response` → `too_slow` → break (`run_benchmark.py:178-182`). Completed answers were accurate. Keep the rung for the size-ceiling datapoint; do not recommend it to users.
