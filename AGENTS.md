...
There is no linter or package manager, and no general test suite. To verify a change, open `index.html` (or `dist/hermit-ui-standalone.html`) in a browser after building. The one exception is `tests/` — focused Export/Import regression tests, because that format doubles as both writer and reader; run `node tests/export-import.test.mjs` (no install needed) and, after a build, `benchmark/.venv/bin/python tests/e2e_export_import.py`. See `tests/README.md`. `dist/hermit-ui-standalone.html`, `dist/hermit-ui-cdn.html`, and `dist/hermit-ui-wllama.html` are committed (browsable on GitHub); `dist/hermit-ui-local.html` and `libs/` are gitignored.
