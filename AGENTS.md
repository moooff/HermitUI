...
- gpt-oss-20b reference (MXFP4, `ggml-org/gpt-oss-20b-GGUF`, same machine, 2026-07-21, 4 clean GPU runs): **12.1 GB loads reliably** in ≈ 78–84 s and decodes at ≈ 43 t/s — *faster* than dense Gemma-4-12B, since only ~3.6B params are active per token. App-reported ≈ 15 t/s at ≈ 3.4 s avg TTFT; the gap is larger than for other models because gpt-oss emits a hidden reasoning trace whose time lands in the app's denominator while its tokens don't (a correct 31-char logic answer read 2.9 t/s app vs 40.5 t/s decode). 3 of 4 runs completed all 10 questions with every answer correct; **1 of 4 stalled** on a writing question — no first token within the harness's hardcoded 120 s TTFT wait (`run_benchmark.py:170-181`, separate from `--question-timeout`) → `no_response` → `too_slow` → break, where the same question took ~4.5 s in the other runs. Cause still unresolved, but the 4th run (`results/20260721-212335-gpu`, 42.6 t/s decode, all answers correct) points at prefill variance rather than a hard hang: TTFT is 1.4–3.9 s for the knowledge/reasoning/code/German questions but spikes on both writing prompts (16.9 s and 9.6 s) while decode stays flat at 41–43 t/s — a >3× spread on the same prompt and hardware, whose tail plausibly overruns the 120 s wait. Not settled at n=4; to resolve, run several passes back to back for the stall rate, and log elapsed time *during* the 120 s wait — it is currently silent while blocking, so a true hang and a 119 s prefill look identical. Worth re-testing when `WLLAMA_CDN_BASE` is next bumped.
